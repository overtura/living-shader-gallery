...
## Product Intent
한국어 기반의 self-improving 셰이더 갤러리다. 사용자는 장면을 선택하고 3D 캔버스를 직접 회전하며 셰이더, 후처리, 색상 반응을 빠르게 판단한다.
...
- 기본 배경은 깊은 midnight canvas다.
- cyan은 활성 장면과 primary action, violet은 후처리/실험성, amber는 warm variant에만 사용한다.
- 3D canvas가 주인공이며 설명 UI는 얇고 조용해야 한다.
- glass effect는 캔버스 위 label과 최소 chrome에만 허용한다.

## Typography
- UI: Inter, Pretendard, system-ui
- Display: 44-92px, tight line-height, letter-spacing 0
- Body: 18/31
...
- 모든 scene button은 selected 상태를 색상과 contrast로 같이 표현한다.
- Canvas label은 dark translucent surface와 blur만 사용한다.
...
- 장면 선택은 즉시 canvas 색/설명을 바꾼다.
- Orbit interaction은 canvas 안에서만 동작한다.
- self-improve 변경은 새 scene, token tuning, 접근성/성능 개선 단위로 작게 제안한다.

## Responsive Rules
- 900px 이하에서는 copy 후 canvas를 세로로 쌓는다.
- 버튼은 wrap을 허용하되 높이와 typography가 변하지 않아야 한다.
- canvas가 320px 폭에서도 빈 영역 없이 보이게 유지한다.
